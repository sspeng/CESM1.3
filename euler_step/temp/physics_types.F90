...
module physics_types
  ...
  logical :: physics_state_alloc_init = .TRUE.;
...
#define SW_PHYSICS_UPDATE
#ifdef SW_PHYSICS_UPDATE
!===============================================================================
...
#define SW_PHYSICS_UPDATE_1
#ifdef SW_PHYSICS_UPDATE_1
    ...
    call t_startf('sw_physics_update')
    call t_startf('physics_update_endrun')
    !-----------------------------------------------------------------------
    ...
    call t_stopf('physics_update_endrun')
    !-----------------------------------------------------------------------
    call t_startf('physics_update_getopts')
    call phys_getopts(state_debug_checks_out=state_debug_checks)
    call t_stopf('physics_update_getopts')

    ncol = state%ncol

    ! Update u,v fields
    call t_startf('physics_update_fields')
    if(ptend%lu) then
    ...
    call t_stopf('physics_update_fields')

   ! Update constituents, all schemes use time split q: no tendency kept
    call t_startf('physics_update_cnst_get_ind')
    ...
    call t_stopf('physics_update_cnst_get_ind')

    call t_startf('physics_update_state_cnst_min_nz')

    !print *, "sw--->>>>>", pcnst, ptend%top_level, ptend%bot_level, ncol, pver, state%psetcols
    do m = 1, pcnst
      if (ptend%lq(m)) then
        ptend_lq(m) = 1
      else
...
#ifdef SW_PHYSICS_UPDATE_1
    call t_startf('sw_physics_update_1')
    param_phy_s1%state_q = loc(state%q)
    param_phy_s1%ptend_q = loc(ptend%q)
    ...
    param_phy_s1%top_level = ptend%top_level
    param_phy_s1%bot_level = ptend%bot_level
    param_phy_s1%lchnk = state%lchnk
    param_phy_s1%psetcols = state%psetcols
    ...
    call athread_join()
    call t_stopf('sw_physics_update_1')
#else
    call t_startf('local_physics_update_1')
    do m = 1, pcnst
       if(ptend_lq(m) == 1) then
          do k = ptend%top_level, ptend%bot_level
          ...
          if (m /= ixnumice  .and.  m /= ixnumliq .and. &
             ...
             call t_startf('physics_update_qnege3')
             !call qneg3(trim(name), state%lchnk, ncol, state%psetcols, pver, m, m, qmin(m), state%q(1,1,m))
             call qneg3_sw(state%lchnk, ncol, state%psetcols, pver, m, m, qmin(m), state%q(1,1,m), nvals(m), iw(m), kw(m), found(m), worst(m))
             call t_stopf('physics_update_qnege3')
          else
             do k = ptend%top_level, ptend%bot_level
    ...
    call t_stopf('local_physics_update_1')
#endif
    ...
    do m = 1, pcnst
      if(ptend_lq(m) == 1) then
        if (m /= ixnumice  .and.  m /= ixnumliq .and. &
            m /= ixnumrain .and.  m /= ixnumsnow ) then
          call qneg3_sw_write(trim(name), state%lchnk, ncol, state%psetcols, pver, m, m, qmin(m), state%q(1,1,m), nvals(m), iw(m), kw(m), found(m), worst(m))
        endif
      endif
    enddo
    ...
    call t_stopf('physics_update_static_energy')
  ...
  if (iam == 0) then
    ...
    do m = 1, pcnst
      do k = ptend%top_level, ptend%bot_level
        do i = 1, ncol
          print *, state%q(i, k, m)
          !call print_binary_physics(state%q(i,k,m))
        end do
      enddo
    enddo
  endif
#endif
    call t_stopf('sw_physics_update')
  contains
...
!---------------------------Local storage-------------------------------
    ...
    call t_startf('local_physics_update')
    !-----------------------------------------------------------------------
    ! Check that the state/tend/ptend are all dimensioned with the same number of columns
    if (state%psetcols /= ptend%psetcols) then
       call endrun('ERROR in physics_update with ptend%name='//trim(ptend%name) &
...
! Update u,v fields
    if (ptend%lu) &
         call multiply_tendency(ptend, ncol, fac, ptend%u, &
         ptend%taux_srf, ptend%taux_top)

    if (ptend%lv) &
         call multiply_tendency(ptend, ncol, fac, ptend%v, &
         ptend%tauy_srf, ptend%tauy_top)

! Heat
    if (ptend%ls) &
         call multiply_tendency(ptend, ncol, fac, ptend%s, &
...
! Update constituents
    ...
    do m = 1, pcnst
       if (ptend%lq(m)) &
            call multiply_tendency(ptend, ncol, fac, ptend%q(:,:,m), &
            ptend%cflx_srf(:,m), ptend%cflx_top(:,m))
    end do


  end subroutine physics_ptend_scale
  subroutine multiply_tendency(ptend, ncol, fac, tend_arr, flx_srf, flx_top)
    ...
    !write(*,*) "printed by in tendency 1",ptend%top_level,ptend%bot_level,ncol
    !return
    do k = ptend%top_level, ptend%bot_level
...
! allocate the individual state components
  ...
  if (physics_state_alloc_init) then
    state%lat(:) = inf
    state%lon(:) = inf
    state%ulat(:) = inf
    state%ulon(:) = inf
