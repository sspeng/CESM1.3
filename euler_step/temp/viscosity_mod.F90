...
#ifdef _PRIM
#define SW_NEIGHBOR_MINMAX
#ifdef SW_NEIGHBOR_MINMAX
subroutine neighbor_minmax(hybrid,edgeMinMax,nets,nete,min_neigh,max_neigh)
  ...
  call t_startf('sw_neighbor_minmax')
...
!real (kind=real_kind) :: Qmax(np,np,nlev,qsize)
   ...
   call t_startf('local_neighbor_minmax')
   kblk = nlev  ! calculate size of the block of vertical levels
   qblk = qsize   ! calculate size of the block of tracers
   ...
   do ie=nets,nete
      ...
      enddo
   enddo
   call t_stopf('local_neighbor_minmax')
end subroutine neighbor_minmax
#endif


#define SW_NEIGHBOR_MINMAX_START
#ifdef SW_NEIGHBOR_MINMAX_START
subroutine neighbor_minmax_start(hybrid,edgeMinMax,nets,nete,min_neigh,max_neigh)
  ...
  call t_startf('sw_neighbor_minmax_start')
  kblk = nlev  ! calculate size of the block of vertical levels
  qblk = qsize   ! calculate size of the block of tracers
  param_edgeSpack_s%qmin = loc(min_neigh)
  param_edgeSpack_s%qmax = loc(max_neigh)
  ...
  call t_stopf('sw_neighbor_minmax_start')
...
subroutine neighbor_minmax_start(hybrid,edgeMinMax,nets,nete,min_neigh,max_neigh)
   ...
   call t_startf('local_neighbor_minmax_start')
   !call get_loop_ranges(hybrid,kbeg=kbeg,kend=kend,qbeg=qbeg,qend=qend)
   kbeg = 1
   kend = nlev
   qbeg = 1
   ...
   call bndry_exchangeS_start(hybrid,edgeMinMax,location='viscosity_mod:882')
   call t_stopf('local_neighbor_minmax_start')
end subroutine neighbor_minmax_start
#endif


#define SW_NEIGHBOR_MINMAX_FINISH
#ifdef SW_NEIGHBOR_MINMAX_FINISH
subroutine neighbor_minmax_finish(hybrid,edgeMinMax,nets,nete,min_neigh,max_neigh)
  ...
  param_edgeSunpack_s%nete = nete
  param_edgeSunpack_s%qsize = qsize
  call t_startf('sw_neighbor_minmax_finish')
  call bndry_exchangeS_finish(hybrid,edgeMinMax,location='viscosity_mod:902')
  call athread_spawn(slave_edgesunpack_es, param_edgeSunpack_s)
  call athread_join()
  call t_stopf('sw_neighbor_minmax_finish')
...
subroutine neighbor_minmax_finish(hybrid,edgeMinMax,nets,nete,min_neigh,max_neigh)
   ...
   call t_startf('local_neighbor_minmax_finish')
   call bndry_exchangeS_finish(hybrid,edgeMinMax,location='viscosity_mod:902')
   ...
   do ie=nets,nete
      ...
      enddo
   enddo
   call t_stopf('local_neighbor_minmax_finish')
end subroutine neighbor_minmax_finish
#endif
