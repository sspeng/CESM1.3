subroutine qneg3_sw (idx, ncol, ncold, lver, lconst_beg, lconst_end, qmin, q, nvals, iw, kw, found, worst)
!-----------------------------------------------------------------------
!
! Purpose:
...
!      if (found(m) .and. nvals(m)>100 .and.  abs(worst(m))>max(qmin(m),1.e-12_r8)) then
!         write(iulog,9000)subnam,m,idx,nvals(m),qmin(m),worst(m),iw(m),kw(m)
!      end if
   end do
!
   return
...
end subroutine qneg3_sw

subroutine qneg3_sw_write(subnam, idx, ncol, ncold, lver, lconst_beg, lconst_end, qmin, q, nvals, iw, kw, found, worst)
!-----------------------------------------------------------------------
!
! Purpose:
! Check moisture and tracers for minimum value, reset any below
...
!-----------------------------------------------------------------------
   ...
   do m=lconst_beg,lconst_end
      if (found(m) == 1 .and. nvals(m)>100 .and.  abs(worst(m))>max(qmin(m),1.e-12_r8)) then
         write(iulog,9000)subnam,m,idx,nvals(m),qmin(m),worst(m),iw(m),kw(m)
      end if
   end do
!
