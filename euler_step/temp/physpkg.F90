...
subroutine phys_run1(phys_state, ztodt, phys_tend, pbuf2d,  cam_in, cam_out)
    ...
    if ( adiabatic .or. ideal_phys )then
       ...
       !print *, "bc physics call tphy", begchunk, endchunk
...
!$OMP PARALLEL DO PRIVATE (C, FLX_HEAT)
    do c=begchunk, endchunk
       ...
       call t_startf("ideal physics_tend_init")
       call physics_tend_init(phys_tend(c))
       call t_stopf("ideal physics_tend_init")
       ! Dump dynamics variables to history buffers
       call t_startf("ideal diag")
       call diag_phys_writeout(phys_state(c))
       call t_stopf("ideal diag")
       if (dycore_is('LR') .or. dycore_is('SE') ) then
          call t_startf("ideal check_energy_fix")
          call check_energy_fix(phys_state(c), ptend(c), nstep, flx_heat)
          call t_stopf("ideal check_energy_fix")
          call t_startf("ideal physics_update")
          call physics_update(phys_state(c), ptend(c), ztodt, phys_tend(c))
          call t_stopf("ideal physics_update")
          call t_startf("ideal check_energy_chng")
          call check_energy_chng(phys_state(c), phys_tend(c), "chkengyfix", nstep, ztodt, &
               zero, zero, zero, flx_heat)
          call t_stopf("ideal check_energy_chng")
          call t_startf("ideal dealloc")
       ...
       call t_startf("ideal pbuf_set_field")
       ...
       call t_stopf("ideal pbuf_set_field")
    end do
